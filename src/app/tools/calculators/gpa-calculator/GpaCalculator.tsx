    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-100 py-8 transition-colors">
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-slate-300">Track and calculate your current or cumulative GPA for school.</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-8 mt-8">
          <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-slate-800 shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-gray-100 dark:border-slate-800">
              <div>
                <label className="block text-sm text-gray-600 dark:text-slate-300 mb-1">Prior Cumulative GPA (Optional)</label>
                <input type="number" step="0.01" min="0" max="5" value={priorGpa || ''} onChange={e => setPriorGpa(Number(e.target.value))} className="w-full rounded-xl border border-gray-300 dark:border-slate-700 p-3" placeholder="e.g. 3.5" />
              </div>
              <div>
                <label className="block text-sm text-gray-600 dark:text-slate-300 mb-1">Prior Credits (Optional)</label>
                <input type="number" min="0" value={priorCredits || ''} onChange={e => setPriorCredits(Number(e.target.value))} className="w-full rounded-xl border border-gray-300 dark:border-slate-700 p-3" placeholder="e.g. 60" />
              </div>
            </div>

            <div className="space-y-4">
              {courses.map(course => (
                <div key={course.id} className="flex flex-wrap gap-2 items-center bg-gray-50 dark:bg-slate-800/60 p-4 rounded-xl border border-gray-100 dark:border-slate-800">
                  <input type="text" placeholder="Course Name" value={course.name} onChange={e => updateCourse(course.id, 'name', e.target.value)} className="flex-1 rounded-xl border border-gray-300 dark:border-slate-700 p-2 min-w-[120px]" />
                  <select value={course.grade} onChange={e => updateCourse(course.id, 'grade', e.target.value)} className="rounded-xl border border-gray-300 dark:border-slate-700 p-2">
                    {Object.keys(GRADE_POINTS).map(g => <option key={g} value={g}>{g}</option>)}
                  </select>
                  <input type="number" min="1" max="10" value={course.credits} onChange={e => updateCourse(course.id, 'credits', Number(e.target.value))} className="w-20 rounded-xl border border-gray-300 dark:border-slate-700 p-2" title="Credits" />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-slate-800 shadow-sm p-6 flex flex-col items-center justify-center text-center">
            <h3 className="text-gray-500 dark:text-slate-400 font-medium mb-2">Cumulative GPA</h3>
            <div className="relative w-48 h-48 flex items-center justify-center mb-4 transition-all duration-500">
              ...
              <div className="absolute text-4xl font-bold text-gray-900 dark:text-white">{gpa.toFixed(2)}</div>
            </div>
            {distinction && (
